...
void sendDataToServer(float lightIntensity) {
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
    ...
    payload += "{\"key\":\"lightIntensity\",\"value\":" + String(lightIntensity) + "}"; // Add lightIntensity object
...
void setup() {
  ...
  if (lightSensor.begin(BH1750::CONTINUOUS_HIGH_RES_MODE)) {
    ...
    Serial.println("Error setting up BH1750!");
    while (1);
  }
}
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    sendDataToServer(lightIntensity);
